...
static int bmi270_get_data(struct bmi270_device *bmi270_device, 
...
static int bmi270_setup(struct bmi270_device *bmi270_device)
{
    ...
    int pwr_conf = 0;
    regmap_read(bmi270_device->regmap, BMI270_REG_PWR_CONF, &pwr_conf);
    printk(KERN_INFO "pwr conf %x\n", pwr_conf); 

    pwr_conf = pwr_conf & 0xfffffffe; 
    regmap_write(bmi270_device->regmap, BMI270_REG_PWR_CONF, pwr_conf);
    ...
    int init_ctrl = 0;
    regmap_write(bmi270_device->regmap, BMI270_REG_INIT_CTRL, 0x0);
    regmap_read(bmi270_device->regmap, BMI270_REG_INIT_CTRL, &init_ctrl);
    printk(KERN_INFO "init ctrl %x\n", init_ctrl);

    printk(KERN_INFO "%lu\n", sizeof(bmi270_config_file));
    regmap_bulk_write(bmi270_device->regmap, BMI270_REG_INIT_DATA, bmi270_config_file, sizeof(bmi270_config_file));

    regmap_write(bmi270_device->regmap, BMI270_REG_INIT_CTRL, 0x1);
    regmap_read(bmi270_device->regmap, BMI270_REG_INIT_CTRL, &init_ctrl);
    printk(KERN_INFO "init ctrl %x\n", init_ctrl);
    ...
    int status = 0;
    regmap_read(bmi270_device->regmap, BMI270_REG_INTERNAL_STATUS, &status);
    printk(KERN_INFO "status %x\n",status);
    if (status & 1) {
        printk(KERN_INFO "Initialization success\n");
    } else {
        printk(KERN_ERR "Initialization failed\n");
        return -1;
    } 

    regmap_write(bmi270_device->regmap, BMI270_REG_PWR_CTRL, 0x0e);
    regmap_write(bmi270_device->regmap, BMI270_REG_ACC_CONF, 0xa8);
    regmap_write(bmi270_device->regmap, BMI270_REG_GYR_CONF, 0xa9);
    regmap_write(bmi270_device->regmap, BMI270_REG_PWR_CONF, 0x02);
